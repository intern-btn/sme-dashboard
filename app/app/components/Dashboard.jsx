    ...
    <div className="min-h-screen p-8 fade-in bg-white">
      ...
      {totalNasional && (
        <div className="bg-white border-2 border-blue-600 rounded-xl p-6 mb-6 shadow-md">
          <h2 className="text-xl font-semibold mb-4 text-blue-600">🎯 TOTAL NASIONAL - {monthInfo.current.fullLabel}</h2>
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-5">
              ...
              <div className="text-xl text-blue-600 font-semibold">{(totalNasional.totalPercent_current || 0).toFixed(2)}%</div>
            </div>
            <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-5">
              ...
              <div className="text-lg text-blue-600 font-semibold">{(totalNasional.kumkPercent_current || 0).toFixed(2)}%</div>
            </div>
            <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-5">
              ...
              <div className="text-lg text-blue-600 font-semibold">{(totalNasional.kurPercent_current || 0).toFixed(2)}%</div>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white border-2 border-gray-200 rounded-xl p-6 shadow-md">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={kanwilData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
              ...
              <Line type="monotone" dataKey="totalPercent_current" stroke="#1976D2" strokeWidth={3} name={`NPL % ${monthInfo.current.name}`} dot={{ fill: '#1976D2', r: 5 }} activeDot={{ r: 8 }} />
            </LineChart>
          </ResponsiveContainer>
          <div className="mt-6 text-center text-gray-500 text-sm border-t pt-4"><p>Klik navigasi di bawah atau tekan → untuk detail | ← untuk realisasi</p></div>
        </div>
      )}
    </div>
  )
