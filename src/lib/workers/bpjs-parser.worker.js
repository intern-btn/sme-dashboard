...
import { parseBPJSIdas, parseBPJSManual, dateFromFilename, buildMonitoringSummary } from '../excel-parsers.js'
...
self.onmessage = async (e) => {
  try {
    ...
    if (manualBuffer) {
      ...
      parsedManual = parseBPJSManual(wb)
    }
    ...
    if (idasBuffer) {
      ...
      if (!masterRows) {
        ...
        try {
          const res = await fetch('/api/data/bpjs/manual_parsed')
          if (res.ok) {
