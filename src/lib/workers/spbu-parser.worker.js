...
import { parseSPBUIdas, parseSPBUManual, dateFromFilename, buildMonitoringSummary } from '../excel-parsers.js'
  ...
  try {
    ...
    if (idasBuffer) {
      ...
      if (!masterRows) {
        ...
        try {
          const res = await fetch('/api/data/prk_spbu/manual_parsed')
          if (res.ok) {
