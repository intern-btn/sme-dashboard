...
import { formatRp, toKOLBucket } from '../../lib/business-utils'

const BTN_BLUE = '#003d7a'
const BTN_ORANGE = '#e84e0f'
const GRAY = '#9CA3AF'
const KOL_COLORS = [BTN_BLUE, BTN_ORANGE, GRAY]

export default function BusinessDashboard({ trend, mergedRows }) {
  ...
  const closedCount = Math.max(0, totalDebitur - foundCount)

  const totalAmtrel = foundRows.reduce((sum, r) => sum + (Number(r?.amtrel) || 0), 0)
  const totalBakiDebet = foundRows.reduce((sum, r) => sum + (Number(r?.bakiDebet) || 0), 0)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          <div className="font-semibold text-gray-900 mb-3">
            Distribusi KOL (yang ditemukan di IDAS)
          </div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={kolPie} dataKey="value" nameKey="name" outerRadius={90} label>
                  {kolPie.map((_, idx) => (
                    <Cell key={idx} fill={KOL_COLORS[idx % KOL_COLORS.length]} />
                  ))}
                </Pie>
                <Tooltip />
              </PieChart>
          ...
          {kolPie.length === 0 && (
            <div className="text-xs text-gray-500 mt-2">
              Belum ada data KOL (IDAS belum diupload).
            </div>
          )}
        </div>
      </div>
...
function MetricCard({ title, value, accent = false, warn = false }) {
  return (
