...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState('monitoring') // 'monitoring' | 'spbu' | 'bpjs' | 'indomaret' | 'productivity'
  ...
  const [productivityFile, setProductivityFile] = useState(null)
  const [productivityUploading, setProductivityUploading] = useState(false)
  const [productivityStep, setProductivityStep] = useState('')
  ...
  const [currentData, setCurrentData] = useState({ npl: null, kol2: null, realisasi: null, realisasi_kredit: null, posisi_kredit: null, prk_spbu: null, bpjs: null, indomaret: null, productivity: null })
  ...
  const handleProductivityUpload = async () => {
    ...
    try {
      ...
      const inp = document.getElementById('productivity-file'); if (inp) inp.value = ''
      setTimeout(() => { fetchCurrentStatus(); fetchHistory() }, 1000)
    } catch (err) {
      setProductivityError(err.message)
    } finally {
      setProductivityUploading(false); setProductivityStep('')
    }
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {activeTab === 'productivity' && (
              <div className="space-y-4">
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-900">
                    <strong>File PRD:</strong> Export sheet <code>PRD</code> dari file DBoards.xlsx.
                    Sheet harus mengandung kolom: RMCode, NIP, Nama, Jabatan, Kantor Cabang, Kantor Wilayah, Target, %Ach, Kategori, dll.
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    File PRD Productivity (.xlsx)
                  </label>
                  <input
                    id="productivity-file"
                    type="file"
                    accept=".xlsx,.xls"
                    onChange={(e) => setProductivityFile(e.target.files[0])}
                    className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                  />
                  {productivityFile && <p className="mt-1 text-sm text-green-600">{productivityFile.name}</p>}
                </div>

                <button
                  onClick={handleProductivityUpload}
                  disabled={productivityUploading || !productivityFile}
                  className={`w-full py-3 rounded-lg font-semibold text-white transition-colors ${
                    productivityUploading || !productivityFile ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-900 hover:bg-blue-800'
                  }`}
                >
                  {productivityUploading ? 'Memproses...' : 'Upload Productivity'}
                </button>
                ...
                {productivityMessage && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-green-700 font-medium">{productivityMessage}</p>
                  </div>
                )}

                {productivityError && (
                  <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
                    <p className="text-red-700">{productivityError}</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 bg-gray-50 rounded border border-gray-200">
                <div className="text-sm font-medium text-gray-900">Productivity</div>
                <div className="text-xs text-gray-600 mt-1">
                  {currentData.productivity ? formatDate(currentData.productivity.uploadedAt) : 'No data'}
                </div>
              </div>
            </div>
          </div>
