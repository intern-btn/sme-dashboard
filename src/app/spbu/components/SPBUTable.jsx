...
export default function SPBUTable({ rows, cabangList, filters, onFiltersChange, idasDate, dataType = 'spbu', isLocked = false, onRefetch }) {
  ...
  const handleExportPDF = () => {
    const headers = [
      'CABANG', 'NO DEBITUR', 'NAMA', 'TGL AKAD', 'TGL JT', 'PLAFON',
      'CMS', 'EDC', 'QRIS', 'BAKI DEBET', 'KOL', 'AMTREL', 'POKOK TERBAYAR', 'STATUS',
    ]
    const data = exportRows.map((r) => [
      r.cabang, r.noDebitur, r.nama, r.tglAkad, r.tglJatuhTempo,
      formatNum(r.plafon), r.cms, r.edc, r.qris,
      formatNum(r.bakiDebet), r.kol, formatNum(r.amtrel),
    ...
    exportTableToPDF({
      ...
      columnStyles: { 2: { cellWidth: 48 }, 0: { cellWidth: 22 }, 1: { cellWidth: 28 } },
    })
  }
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[1200px] w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {sortedRows.map((r, idx) => {
              ...
              return (
                <tr key={r?._noDebiturHash || r?.noDebitur || idx} className={`${rowClass} hover:bg-gray-50`}>
                  ...
                  <td className="px-3 py-2 text-center">{r?.hasCMS ? CHECK : ''}</td>
                  <td className="px-3 py-2 text-center">{r?.hasEDC ? CHECK : ''}</td>
                  <td className="px-3 py-2 text-center">{r?.hasQRIS ? CHECK : ''}</td>
            ...
            {sortedRows.length === 0 && (
              <tr><td colSpan={14} className="px-4 py-10 text-center text-gray-400">Tidak ada data.</td></tr>
            )}
          </tbody>
        </table>
      </div>
